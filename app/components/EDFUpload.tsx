...
export default function EDFUpload({ isDarkMode = false, onFileUploaded }: EDFUploadProps) {
  ...
  const uploadFiles = async (filesToUpload: File[]) => {
    ...
    try {
      for (const file of filesToUpload) {
        ...
        const response = await fetch('http://localhost:8000/api/edf-files/upload/', {
          method: 'POST',
          body: formData,
        });
      ...
      alert('Upload failed. Please check if the Django backend is running.');
    } finally {
      setUploading(false);
    }
  };
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <p className={`text-sm ${
                ...
                Supports .edf and .bdf files up to 100MB each
              </p>
            </div>
          </div>
        )}
